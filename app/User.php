...
class User extends Model implements Authenticatable, JWTSubject
{
    ...
    private function setTokens($result)
    {
        ...
        if (!in_array($this->tokenData->role, ['Student', 'Tutelary'])) {
            throw new UnexpectedValueException('Csak diákok és gondviselők használhatják az alkalmazást!');
        }
        $this->save();
    }
    ...
    public function loadExams()
    {
        return DB::table('exams')->where([
            ['user_id', '=', $this->id],
            ['date', '>', new Expression('current_date')]
